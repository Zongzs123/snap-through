...
def train(args):
    ...
    if args.flag == 1:
        ...
        random_search = RandomizedSearchCV(forward_model, param_distributions=param_dist, n_iter=25, cv=5, scoring=scorer,
                                           verbose=2)
    else:
        # Create the GridSearchCV object with the callbacks
        random_search = GridSearchCV(forward_model, param_dist, cv=5, scoring=scorer, verbose=2)
...
if __name__ == '__main__':
    ...
    args.run_name = "./forward_network_gird/20240514/"
    args.epochs = 200
    args.batch_size = 64
